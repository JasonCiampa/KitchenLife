 ...
 * @author Jason Ciampa
 */
public abstract class Booth extends Asset implements Interactable {
    ...
    protected float seatX;                                                                                                                          // The x-coordinate of the seat to sit the Player down in
    protected float seatY;                                                                                                                          // The y-coordinate of the seat to sit the Player down in
    protected float seatZ;                                                                                                                          // The z-coordinate of the seat to sit the Player down in
        
    protected Asset[] plates;                                                                                                                       // An array of Plate objects that the Booth can store
    protected Food[] food;                                                                                                                          // An array of Food objects that the Booth can store
    ...
    public Booth(float x, float y, float z, int speedReward, Food food) {
      ...
      this.food = new Food[4];                                                                                                                                                             // Initialize the Food array with a size of 4
      
      for (int i = 0; i < 4; i++) {                                                                                                                                                        // For 4 iterations...
          this.food[i] = new Food(food.gltfFilePath, this.plates[i].x - 2.75f, this.plates[i].y + 14.25f, this.plates[i].z - 8, food.length, food.width, food.height); // Add this Booth's food to the array
      } 
    }
    ...
    @Override
    // Detects any interactions that are being made on this Booth
    public void detectInteractions() {
        ...
        if (distanceFromPlayer < (this.width + 2)) {                                                                                                                                      // If the Player is close enough to the edge of the Booth...
            ...
            if (Gdx.input.isKeyPressed(Input.Keys.E)) {                                                                                                                                  // If the 'e' key is pressed...
                this.processInteraction(player);                                                                                                                                          // Trigger the Player's interaction on the Booth
            }
        }
    }

    @Override
    // Process an interaction that has been made on this Booth
