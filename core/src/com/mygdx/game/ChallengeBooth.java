...
public class ChallengeBooth extends Booth {
    ...
    protected double timeLeft;                                                                                                                                                                                   // How much time the player has left to eat their meal  (measured in seconds)
    ...
    protected void processEatingRun(float dt) {
        if (this.oneSecondTimer > 0) {                                                                                                                                                                           // If the one second timer is still going...
            ...
            this.bitesRemaining -= (player.getEatingSpeed() + player.applyBonusBites() * (int)(player.getEatingSpeed() * 0.1d));                                                                                       // Decrement the # of bitesRemaining by the calculated number of bites taken this past second  (Not sure if this math will make it balanced in-game, but we'll test and adjust)
        }
    ...
    protected void endEatingRun(boolean playerWon) {
        ...
        if (playerWon) {                                                                                                                                                                                         // If the Player was victorious...
            ...
            drawer.displayMessage("You completed the Restaurant's Challenge and were rewarded with speed and reputation!", 250, 100, 1420, 200, 0.1f);     // Display a message saying that the Player won
        }
        else {                                                                                                                                                                                                   // Otherwise, the Player must have lost, so...
            drawer.displayMessage("You failed the Restaurant's Challenge. Increase your eating speed and try again!", 250, 100, 1420, 200, 0.1f);              // Display a message saying that the Player lost
        }
    }
